...
We now have all that is needed to begin downloading data, but before we do so we can do a quick visual inspection to see how far away the selected weather stations are from the location of interest. The code below for the `multiplot` function was developed by << insert refernce >> and is used in the <<  >> package.  However, instead of increasing the number of dependencies of this package, we decided to copy the code for internal use.
