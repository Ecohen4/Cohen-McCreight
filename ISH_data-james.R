...
st <- read.csv(file=paste("/Users/elliotcohen/Dropbox/data/cohen-mccreight/data/raw", files[85], sep=""))
head(st)
dim(st)
...
dates <- seq(as.Date("2011-06-21"), as.Date("2011-09-21"), 1)
for (i in 1:length(files)) {
  ...
  st <- read.csv(paste("/Users/elliotcohen/Dropbox/data/cohen-mccreight/data/raw", files[i], sep=""))
  st$WIND.DIR <- st$DEW.POINT <- st$ATM.PRES <- NULL
  st$DATE <- as.Date(paste(st$YR, st$M, st$D, sep="-"), format="%Y-%m-%d")
  st <- st[st$DATE >= "2011-06-21" & st$DATE <= "2011-09-21",]
  st$TEMP[st$TEMP == 999.9] <- NA
  st$WIND.SPD[st$WIND.SPD == 999.9] <- NA
